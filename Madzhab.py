...
'''
            ...
            poin = 0
            garis()
            print(Fore.LIGHTWHITE_EX + Style.BRIGHT + "Materi Madzhab:")
            print(materi_madzhab)
            garis()
            for i, tanya in enumerate(pertanyaan, 1):
                jawaban = input(Fore.CYAN + f"{i}. {tanya} ")
                # Cek jawaban (case insensitive & hapus spasi berlebih)
                if jawaban.lower().strip() == jawaban_benar[i-1]:
                    poin += 20
                    print(Fore.GREEN + "Jawaban benar! +20 poin")
                else:
                    print(Fore.RED + f"Jawaban salah! (Jawaban benar: {jawaban_benar[i-1]})")
            garis()
            print(Fore.YELLOW + Style.BRIGHT + f"Total poin: {poin}")
            garis()
...
'''
            garis()
            print(Fore.LIGHTWHITE_EX + Style.BRIGHT + "Sejarah Madzhab:")
            ...
            print(Fore.LIGHTRED_EX + "\nTerima kasih telah menggunakan Madzhab Tool!")
            break
        else:
